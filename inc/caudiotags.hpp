...
namespace AudioTag
{
    ...
    typedef struct __generic_tag
    {
        ...
        __generic_tag() : title(""), artist(""), album(""),
            comment(""), artfile(""), track(0), year(0) { }

        __generic_tag(std::string title, std::string artist, std::string album,
                      std::string comment, std::string artfile, unsigned int track,
                      unsigned int year) : title(title), artist(artist), album(album),
